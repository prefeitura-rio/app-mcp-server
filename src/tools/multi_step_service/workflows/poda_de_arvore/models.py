...
class EmailPayload(BaseModel):
    email: Optional[str] = Field(None, description="Email válido do usuário (opcional)")
...
class AddressData(BaseModel):
    ...
    complemento: Optional[str] = Field(
        None, description="Apartamento, bloco, etc (opcional)"
    )
    ponto_referencia: Optional[str] = Field(
        None, description="Ponto de referência (opcional)"
    )
