...
from src.tools.multi_step_service.workflows.poda_de_arvore.state_helpers import (
    ticket_opened,
    ticket_failed,
)
    ...
    def _handle_address_from_memory(self, state: ServiceState) -> bool:
        ...
        if (
            ...
            state.agent_response = AgentResponse(
                description=tpl.endereco_historico(
        ...
        if state.payload and "address" in state.payload:
            try:
                ...
                if not address_info.get("valid"):
                    ...
                    state.agent_response = AgentResponse(
                        description=tpl.endereco_nao_localizado(
                ...
                if validation_state.attempts >= validation_state.max_attempts:
                    ...
                    state.agent_response = AgentResponse(
                        description=tpl.endereco_erro_processamento(
                            validation_state.attempts, validation_state.max_attempts
                        ),
                        payload_schema=AddressPayload.model_json_schema(),
    ...
    async def _collect_cpf(self, state: ServiceState) -> ServiceState:
        ...
        should_ask_about_data = (
            ...
            or (
                state.data.get(
                    "identificacao_pulada"
                )  # Verifica a flag ao invés do valor do CPF
        ...
        if "cpf" in state.data and "cpf" not in state.payload:
            if (
                state.data.get(
                    "identificacao_pulada"
                )  # Verifica a flag ao invés do valor
        ...
        if state.payload and "email" in state.payload:
            ...
            try:
                ...
                if attempts >= 3:
                    ...
                    logger.warning(
                        f"Email: máximo de tentativas ({attempts}) excedido. Pulando email."
                    )
                    state.agent_response = AgentResponse(
        ...
        if state.payload and "name" in state.payload:
            ...
            try:
                ...
                if attempts >= 3:
                    ...
                    logger.warning(
                        f"Nome: máximo de tentativas ({attempts}) excedido. Pulando nome."
                    )
    ...
    async def _open_ticket(self, state: ServiceState) -> ServiceState:
        ...
        try:
            ...
            return ticket_failed(
                ...
                description=tpl.solicitacao_existente(
        ...
        if "confirmacao" in state.payload:
            try:
                ...
                if validation_state.attempts >= validation_state.max_attempts:
                    ...
                    state.agent_response = AgentResponse(
                        description=tpl.solicitar_novo_endereco(
                            validation_state.attempts, validation_state.max_attempts
                        ),
                        payload_schema=AddressPayload.model_json_schema(),
                    )
